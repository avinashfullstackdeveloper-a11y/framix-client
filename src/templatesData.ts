...
// Caching logic for templatesData
function getCachedTemplatesData(): Template[] {
 const cached = localStorage.getItem("templatesDataCache");
 if (cached) {
   try {
 ...
 // If not cached, cache the default and return it
 localStorage.setItem("templatesDataCache", JSON.stringify(defaultTemplatesData));
...
// Export templatesData, loading from cache if available
export const templatesData: Template[] = getCachedTemplatesData();