...
const Components = () => {
  const filterTabs = ["ALL", "HTML", "CSS", "Figma", "JavaScript", "Image"];
  ...
  // Dynamically generate image components from c1.png to c12.png
  const imageComponents = Array.from({ length: 12 }, (_, i) => ({
    id: i + 1,
    title: `Image ${i + 1}`,
    type: "image",
    src: `/assets/images/c${i + 1}.png`,
    gradient: false
  }));

  const componentItems = [...existingComponents, ...imageComponents];
                ...
                {item.type === "button" && (
                  <Button 
                    className={item.gradient 
                      ? "bg-gradient-primary hover:opacity-90 border-0 text-lg px-8 py-3 transform hover:-translate-y-1 transition-transform duration-300 ease-out shadow-md hover:shadow-lg" 
                      : "bg-secondary hover:bg-muted border-border text-lg px-8 py-3 transform hover:-translate-y-1 transition-transform duration-300 ease-out shadow-md hover:shadow-lg"
                    }
                  >
                    {item.title}
                  </Button>
