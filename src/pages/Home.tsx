...
description: "Framix provides ready-to-use components and templates for beginners, while offering flexibility and depth for advanced designers and developers."
},
{
title: "Industry Best Practices.",
description: "Our components are designed following the latest standards for accessibility, responsiveness, and performance."
},
{
title: "Trusted & Reliable",
description: "Framix is crafted to deliver stable, high-quality performance, empowering teams to build faster with confidence."
}
];
...
description: "With industry-standard practices and seamless scalability, Framix helps teams deliver high-performing projects confidently."
},
{
icon: "🎛️",
title: "Customize with ease",
description: "Tailor every detail to your brand with pixel-perfect control and modern design standards."
},
{
icon: "🚀",
title: "Launch faster",
description: "Deploy beautiful, functional designs in record time without reinventing the wheel."
}
];
...
const trustedPlatformImages = Array.from({ length: 20 }, (_, i) => `/assets/images/i${i + 1}.png`);
...
<motion.div className="text-center lg:text-left z-10" variants={itemVariants}>
<h1 className="text-5xl lg:text-6xl font-bold mb-6 leading-tight">
Accelerate products with <span className="text-[#FF94C666]">ready-made components</span>
</h1>
<p className="text-xl text-gray-400 mb-8 leading-relaxed">
Build modern interfaces faster without starting from scratch. Ready-to-ship components and templates to help you build beautiful products faster.
</p>
...
<Link to="/components">
<Button variant="outline" className="border-gray-600 text-white hover:bg-gray-800 hover:text-white font-semibold px-6 py-3 rounded-md">
Explore Components
</Button>
</Link>
</div>
...
<img src="/phone.png" alt="App Preview" className="w-[300px] md:w-[400px] relative z-10" />
</motion.div>
</div>
</motion.section>
...
style={{
transform: `translateX(${(index - activeTrustedCard) * 10}%) ${index !== activeTrustedCard ? 'scale(0.95)' : ''}`,
zIndex: trustedWayCardData.length - Math.abs(index - activeTrustedCard),
}}
>
<CardContent className="p-0">
...
<a href="#" className="text-sm font-semibold mt-4 inline-block">Learn more →</a>
</motion.div>
</AnimatePresence>
)}
</CardContent>
...
{[...Array(5)].map((_, i) => (
<svg key={i} className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
))}
</div>
...
<motion.div className="h-96 overflow-hidden relative" variants={itemVariants}>
<motion.div
className="grid grid-cols-4 gap-4"
animate={{ y: ['0%', '-50%'] }}
transition={{ ease: 'linear', duration: 30, repeat: Infinity }}
>
{[...trustedPlatformImages, ...trustedPlatformImages].map((src, i) => (
<img key={i} src={src} alt={`Component preview ${i + 1}`} className="h-32 w-full object-cover rounded-lg" />
))}
</motion.div>
</motion.div>
</div>
...
Framix provides everything you need to create, launch, and scale high-quality projects – all powered by flexible UI components.
</motion.p>
...
>
<Card className={`bg-[#1c1c1c] border-gray-800 p-8 text-center flex flex-col justify-start items-center h-full transition-all duration-300 ease-in-out ${activePortfolioCard === index ? 'bg-[#FF94C666]/90' : ''}`}>
<CardContent className="p-0">
<div className={`w-20 h-20 bg-gray-800 rounded-full flex items-center justify-center mb-6 transition-all duration-300 ${activePortfolioCard === index ? 'bg-white/20' : ''}`}>
<span className="text-4xl text-[#FF94C666]">{card.icon}</span>
</div>
...
<Button className="bg-black text-white mt-6">Learn More</Button>
</motion.div>
)}
</AnimatePresence>
</CardContent>
...
<div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
<motion.div variants={itemVariants}>
<h2 className="text-4xl font-bold mb-6">What's new we provide for you?</h2>
<p className="text-gray-400 mb-8">Framix delivers a growing ecosystem of powerful UI components, templates, and tools designed to help designers and developers ship projects faster.</p>
<div className="space-y-6">
<div className="flex items-center gap-4">
<CheckCircle2 className="w-8 h-8 text-[#FF94C666]" />
<div>
<h3 className="font-semibold">We provide production-ready UI blocks</h3>
<p className="text-gray-500 text-sm">that save hours of design & coding.</p>
</div>
</div>
<div className="flex items-center gap-4">
<CheckCircle2 className="w-8 h-8 text-[#FF94C666]" />
<div>
<h3 className="font-semibold">Easily connect Framix components</h3>
<p className="text-gray-500 text-sm">with your tech stack.</p>
</div>
</div>
</div>
</motion.div>
<motion.div className="relative h-96 w-full" variants={itemVariants}>
<AnimatePresence>
{whatsNewVideos.map((videoSrc, index) => (
<motion.div
key={videoSrc}
className="absolute w-full h-full"
initial={{ opacity: 0, y: 50, scale: 0.9, rotate: (index - activeWhatsNewCard) * 5 }}
animate={{
opacity: 1,
...
style={{ zIndex: whatsNewVideos.length - Math.abs(index - activeWhatsNewCard) }}
>
<Card className="w-full h-full bg-white text-gray-800 p-2 rounded-2xl shadow-2xl overflow-hidden">
<video src={videoSrc} autoPlay loop muted className="w-full h-full object-cover rounded-xl"></video>
</Card>
</motion.div>
))}
</AnimatePresence>
  ...
  <div className="relative overflow-hidden w-full">
    ...
    >
      {[...testimonialsData, ...testimonialsData].map((testimonial, index) => (
        <div key={index} className="flex-shrink-0 w-full sm:w-1/2 lg:w-1/4 p-3">
